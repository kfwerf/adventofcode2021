...

{
    ...
    const drawnNumbers = randomOrder.map((_, idx, arr) => arr.slice(0, idx)).filter(arr => !!arr.length)
    const allResults = drawnNumbers.map((range, roundNumber) => {
        return range.reduce((prevPuzzles, nextDrawnNumber) => {
            const newPuzzles = [...prevPuzzles.puzzles]
                .map(({ puzzle: oldPuzzle } = obj) => {
                    const newPuzzle = oldPuzzle.map((row) => row.map(col => col === nextDrawnNumber ? NaN : col));
    ...
    const orderedResults = allResults
        .map((obj, idx) => ({ ...obj, puzzles: obj.puzzles.map((obj, i) => ({ ...obj, i })), round: idx }) )
        
    const allWon = orderedResults.filter(({ puzzles } = obj) => puzzles.filter(puzzle => puzzle.hasWon).length === puzzles.length)[0]
    const lastPuzzleIndex = orderedResults[allWon.round-1].puzzles.filter((puzzle) => !puzzle.hasWon)[0].i;
    const lastPuzzle = allWon.puzzles[lastPuzzleIndex];
    
    console.log(lastPuzzle.score)
}
